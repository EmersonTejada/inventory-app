...
const camelCaseFormat = `id, title, description, price, stock, category_id AS "categoryId", created_at AS "createdAt", updated_at AS "updatedAt"`;
...
export const updateProduct =  async (id: number, product: Partial<NewProduct>) => {
    ...
    if (result.rowCount === 0) {
        throw new NotFoundError(`No existe un producto con el id ${id}`)
    }
    return result.rows[0]
}
